...
def insert_account_log_v2(port, action, remark):
    ...
    with UsingMysql() as um:
        ...
        um.cursor.execute(sql)
        um._conn.commit()
def insert_account_v3(action,remark):
    with UsingMysql() as um:
        sql = "INSERT INTO account_log ( `action`, `remark`, `create_date`) VALUES ('{}','{}',now())".format(
            action,remark)
        um.cursor.execute(sql)
        um._conn.commit()
...
def update_record_info(origin_text, item_id, task_id, task_label,sku):
    ...
    with UsingMysql()as um:
        if sku is not None:
            sql = "update product_record set  original_info='{}',`status`=2 where item_id='{}'and sku_id='{}' and task_id='{}' and task_label='{}'  and `status`=0".format(
                origin_text, item_id,sku, task_id, task_label)
        else:
